      ...
      if (out_res.net_boxes)
      {
        if (align_face(out_res.net_boxes, image_matrix, aligned_face) == ESP_OK)
        {
          ...
          if (g_state == START_DETECT) {
            ...
            //Serial.println(int(image_matrix));
           // Serial.println(int(out_res.image));
          }
          ...
          if (g_state == START_RECOGNITION  && (st_face_list.count > 0))
          {
            ...
            //Serial.println(int(out_res.face_id));
            if (f)
            {
              char recognised_message[64];
              sprintf(recognised_message, "RECOGNISED %s", f->id_name);
              client.send(recognised_message);
              //rgb_printf(image_matrix, FACE_COLOR_GREEN, "Hello %u", f->id_name);
            }
            else
            {
